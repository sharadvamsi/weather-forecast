...
const displayFutureForecast = (forecastList) => {
  if (!forecastList || forecastList.length === 0) {
    alert("Failed to fetch future forecast data");
    return;
  }

  const forecastDiv = document.getElementById("future-forecast");
  ...
  forecastDiv.innerHTML = forecastList
    .filter((day) => {
      ...
      return formattedDate >= formattedTomorrow; // Exclude today, start from tomorrow
    })
    .filter((_, index) => index % 8 === 0) // Take every 8th forecast item
